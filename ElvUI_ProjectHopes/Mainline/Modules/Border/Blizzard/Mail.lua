...
local function Skin_OpenMail()
	for i = 1, _G.ATTACHMENTS_MAX_RECEIVE do
		...
		if btn.template then
			BORDER:CreateBorder(btn)        
			BORDER:HandleIconBorder(btn.IconBorder, btn.border)
		end
	end
end
...
function S:MailFrame()
	...
	_G.InboxFrame.backdrop:Kill()

	BORDER:CreateBorder(_G.MailFrameTab1, nil, nil, nil, nil, nil, true, true)
	BORDER:CreateBorder(_G.MailFrameTab2, nil, nil, nil, nil, nil, true, true)

	-- Reposition Tabs
	_G.MailFrameTab1:ClearAllPoints()
	_G.MailFrameTab2:ClearAllPoints()
	_G.MailFrameTab1:Point('TOPLEFT', _G.MailFrame, 'BOTTOMLEFT', -3, -5)
	_G.MailFrameTab2:Point('TOPLEFT', _G.MailFrameTab1, 'TOPRIGHT', 0, 0)

	-- send mail
	BORDER:CreateBorder(_G.SendMailScrollFrame)
	BORDER:CreateBorder(_G.SendMailScrollFrame.ScrollBar.Track.Thumb, nil, nil, nil, nil, nil, true, true)
	...
	-- open mail (cod)
	BORDER:CreateBorder(_G.OpenMailFrame)
	...
	BORDER:CreateBorder(_G.OpenMailScrollFrame.ScrollBar.Track.Thumb, nil, nil, nil, nil, nil, true, true)
