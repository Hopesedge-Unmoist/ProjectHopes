...
local function UpdateKeybindButtons(self)
	...
	for panel in self.bindingsPool:EnumerateActive() do
		if not panel.IsBorder then
			BORDER:CreateBorder(panel.Button1, nil, nil, nil, nil, nil, false, true)
			BORDER:CreateBorder(panel.Button2, nil, nil, nil, nil, nil, false, true)
			if panel.CustomButton then
				BORDER:CreateBorder(panel.CustomButton, nil, nil, nil, nil, nil, false, true)
			end
...
local function HandleDropdown(option)
	BORDER:CreateBorder(option.Dropdown, nil, nil, nil, nil, nil, false, true)
...
local function HandleControlGroup(controls)
	for _, child in next, { controls:GetChildren() } do
		...
		if child.Checkbox then
			BORDER:CreateBorder(child.Checkbox, nil, nil, nil, nil, nil, true, true)
		end
