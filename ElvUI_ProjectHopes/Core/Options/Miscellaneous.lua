...
function ProjectHopes:Miscellaneous()
    ...
    POAMSA.gcd.args.settings.args.keepRatio = ACH:Toggle(L["Keep Icon Ratio"], L["Keep icons square shaped. When disabled, you can set custom width and height for each icon type."], 3, nil, false, nil, function() return E.db.ProjectHopes.gcd.keepRatio end, function(_, value) E.db.ProjectHopes.gcd.keepRatio = value if SpellHistory then SpellHistory:UpdateMainIconSize() SpellHistory:UpdateHistoryIconSize() end end)
