...
function ID:ConstructFrame()
    ...
    local frame = CreateFrame("Frame", "PHInstanceDifficultyFrame", _G.Minimap)
    frame:Size(30, 20)
    frame:Point("TOPLEFT", M.MapHolder, "TOPLEFT", 10, -10)

    local text = frame:CreateFontString(nil, "OVERLAY")
    -- Get the user's font settings
    local fontName = E.LSM:Fetch("font", E.db.ProjectHopes.minimapid.font.name)
    local fontStyle = E.db.ProjectHopes.minimapid.font.style
    local fontSize = E.db.ProjectHopes.minimapid.font.size or 20
    ...
    E:CreateMover(
        ...
        "ALL,PROJECTHOPES",
        function()
            return E.db.ProjectHopes.minimapid.difficulty.enable
        end,
        "ProjectHopes,maps"
    )
...
function ID:Initialize()
    ...
    self.db = E.db.ProjectHopes.minimapid
