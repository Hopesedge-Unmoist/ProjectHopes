...
function B:ConstructContainerFrame(name, isBank)
	...
	if E.db.ProjectHopes.skins.bag then
		--Sort Button
...
function B:ConstructContainerButton(f, bagID, slotID)
	...
	if E.db.ProjectHopes.skins.bag then
		...
		slot.border:SetBackdrop(Private.BorderLight)
	end
    return slot
end
...
function B:UpdateSlotColors(slot, isQuestItem, questId, isActiveQuest)
	...
	if E.db.ProjectHopes.skins.bag then
		...
		if not r then r, g, b = 132/255, 132/255, 132/255 end
...
function S:ElvUI_Bags()
	...
	if not E.db.ProjectHopes.skins.bag then return end
	...
	if E.Classic then
		--BORDER:CreateBorder(_G.ElvUI_ContainerFrameKeyButton, nil, nil, nil, nil, false, true)
	end
