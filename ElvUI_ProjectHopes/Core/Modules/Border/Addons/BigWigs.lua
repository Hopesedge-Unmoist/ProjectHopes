...
local function removeStyle(bar)
    ...
    local spark = bar:Get("bigwigs:hopesui:spark")
    if spark then
        spark:Hide()
        table.insert(pool.spark, spark)
        bar:Set("bigwigs:hopesui:spark", nil)
    end
end
...
local function styleBar(bar)
    ...
    spark:SetTexture(Private.Glowline)
    ...
    spark:Show()
    bar:Set("bigwigs:hopesui:spark", spark)
    ...
    if tex then
        ...
        iconBd:SetBackdrop(Private.Border)
...
function S:BigWigs_Plugins()
	...
	BigWigsAPI:RegisterBarStyle("ProjectHopes", {
		apiVersion = 1,
		version = 1,
		barSpacing = 8,
		barHeight = 20,
		...
		GetStyleName = function() return "ProjectHopes" end,
	})
end

function S:BigWigs_QueueTimer()
	if not E.db.ProjectHopes.skins.bigwigsqueue then return end
			...
			function(_, frame, name)
				if name == "QueueTimer" then
					...
					if spark then
						...
						frame.spark:SetTexture(Private.Glowline)
					...
					local fontPath = [[Interface\AddOns\ElvUI_ProjectHopes\Media\Fonts\Expressway.ttf]]
...
--[[ Work in progress to get BigWigs nameplates to have border. 
function S:BigWigs_Nameplate()
    if _G.BigWigsLoader then
        print("BigWigsLoader found, registering message.")
        _G.BigWigsLoader.RegisterMessage(
            "ElvUI_ProjectHopes",
            "BigWigs_StartNameplate",
            function(event, module, guid, key, length, customIconOrText)

            end
        )
    else
        print("BigWigsLoader not found.")
    end
end

S:AddCallback("BigWigs_Nameplate")
]]
